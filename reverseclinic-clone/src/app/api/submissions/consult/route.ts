...
import { insertConsultRequest } from "@/lib/reverse-db";
...
export async function POST(request: Request) {
  ...
  const body = (await request.json().catch(() => null)) as ConsultPayload | null;
  const name = body?.name?.trim();
  const phone = body?.phone?.trim();
  const branch = body?.branch?.trim() ?? "";
  const message = body?.message?.trim();
  const requestType = body?.requestType?.trim() || "간편 문자상담";
  const sourceSlug = body?.sourceSlug?.trim() || "";
  const locale = body?.locale ?? "ko";
  const pagePath = body?.pagePath?.trim() || "/";

  if (!name || !phone || !message) {
    return badRequest("상담 요청에 필요한 정보를 입력해 주세요.");
  }

  insertConsultRequest(tenantId, {
    name,
    phone,
    branch,
    message,
